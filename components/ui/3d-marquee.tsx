...
export function ThreeDMarquee({ images, className }: ThreeDMarqueeProps) {
  ...
  return (
    ...
    >
      <div className="flex h-full w-full justify-center gap-3 md:gap-20 [transform:rotateX(20deg)_rotateZ(-10deg)]">
...
function MarqueeColumn({
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-12 overflow-hidden">
      <motion.div
      ...
      >
        {duplicatedImages.map((src, idx) => (
          <div
            key={idx}
            className="group relative h-[160px] w-[105px] sm:h-[400px] sm:w-[300px] md:h-[750px] md:w-[580px] flex-shrink-0 overflow-hidden rounded-xl sm:rounded-3xl border border-white/10 bg-zinc-900 transition-all duration-700 hover:scale-105 hover:border-[#c5a059]/60 shadow-[0_25px_60px_rgba(0,0,0,0.9)]"
          >
            <img
              src={src}
              alt={`Galeria ${idx}`}
